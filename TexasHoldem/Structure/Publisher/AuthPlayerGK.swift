...
//  TexasHoldem
//
//  Created by Calogero Friscia on 17/12/21.
//

import Foundation
import GameKit

class AuthPlayerGK: ObservableObject {
    
    static let instance:AuthPlayerGK = AuthPlayerGK()
    
    var localPlayerAuth:Bool = false
   // static var achievCount:Int = 0
    ...
    init() {
        ...
        authenticateUser()
        print("init AUTHPLAYERGK")
    }
    
    var achievementsDone:[String] = []
    
    @Published var isLocked:(level2:Bool?,level3:Bool?,level4:Bool?)
    @Published var showError:Bool?
    @Published var error:String?
    
    func authenticateUser() {

        print("localPlayer isAuth in AuthPlayerGK: \(GKLocalPlayer.local.isAuthenticated.description)")
        self.isLoading = true
        
        GKLocalPlayer.local.authenticateHandler = {vc, error in
             
             print("INSIDE AUTHENTICATEHANDLER in AuthPlayerGK")
             ...
             guard error == nil else {
               ...
               //  self.defaultBankroll()
                // GameAction.authFailed = true
                 ...
                 print("Error != nil -> is Player locale autenticato in AuthPlayerGK: \(GKLocalPlayer.local.isAuthenticated.description) - bankroll to 200 ")
            ...
            self.loadAchievementDone()
            GKAccessPoint.shared.isActive = true // accediamo direttamente all'access point. Per il resto del gioco, per spegnere e accendere l'accessPoint useremo l'apposita funzione creata nella GameAction alla quale da qui non possiamo accedere

           // self.ifLocalPlayerIsAuth()
          //  self.isLoading = false
            print("\(GKLocalPlayer.local.displayName) - isAuth in AuthPlayerGK: \(GKLocalPlayer.local.isAuthenticated.description)")
            print("dentro if localPlayer.isAuth in AuthPlayerGK")
         }
     }
  ...
  /* GKAchievement.resetAchievements { error in
            ...
            GKAchievement.loadAchievements { achievements, error in
                ...
                print("achiCount: \(self.achievementsDone.count) - achi:\(self.achievementsDone)")
                ...
                self.isLoading = false
              //  AuthPlayerGK.achievCount = self.achievementsDone.count
                print("loadAchievement Done")
            }
